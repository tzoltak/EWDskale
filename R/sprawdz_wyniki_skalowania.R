...
sprawdz_wyniki_skalowania = function(nazwaPliku) {
  ...
  for (i in obiekty) {
    model = get(i)
    rm(list = i)
    if (!("wynikiSkalowania" %in% class(model))) {
      next
    } else if (!("oszacowania") %in% names(model)) {
      warning("W pliku '", nazwaPliku, "' obiekt '", i,
              "' jest klasy 'wynikiSkalowania, ale nie zawiera elementu 'oszacowania'.\n",
              "Nie można przeprowadzić diagnostyki tego modelu.\n", immediate. = TRUE)
      next
    }
